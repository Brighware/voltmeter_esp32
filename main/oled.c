...
#define EXAMPLE_PIN_NUM_SDA           OLED_PIN_SDA
#define EXAMPLE_PIN_NUM_SCL           OLED_PIN_SCL
#define EXAMPLE_PIN_NUM_RST           -1
#define EXAMPLE_I2C_HW_ADDR           OLED_I2C_ADDR


#define EXAMPLE_LCD_V_RES LCD_V_RES
#define EXAMPLE_LCD_H_RES LCD_H_RES
...
static bool oled_notify_lvgl_flush_ready(esp_lcd_panel_io_handle_t io_panel, esp_lcd_panel_io_event_data_t *edata, void *user_ctx)
{
...
static void oled_lvgl_flush_cb(lv_display_t *disp, const lv_area_t *area, uint8_t *px_map)
{
...
static void oled_lvgl_port_task(void *arg)
{
...
void oled_demo_ui(lv_display_t *disp)
{
    lv_obj_t *scr = lv_display_get_screen_active(disp);
    lv_obj_t *label = lv_label_create(scr);
    lv_label_set_long_mode(label, LV_LABEL_LONG_SCROLL_CIRCULAR); /* Circular scroll */
    lv_label_set_text(label, "Hello Espressif, testing 1-2-3.");
...
void print_text(lv_display_t *disp, char* txt){
    lv_obj_t *scr = lv_display_get_screen_active(disp);
    lv_obj_t *label = lv_label_create(scr);
    lv_label_set_long_mode(label, LV_LABEL_LONG_SCROLL_CIRCULAR); /* Circular scroll */
...
void init_oled(void) {
    ...
    lv_display_set_flush_cb(display, oled_lvgl_flush_cb);
    ...
    xTaskCreate(oled_lvgl_port_task, "LVGL", EXAMPLE_LVGL_TASK_STACK_SIZE, NULL, EXAMPLE_LVGL_TASK_PRIORITY, NULL);

    ESP_LOGI(TAG, "Display LVGL Scroll Text");
    // Lock the mutex due to the LVGL APIs are not thread-safe
    _lock_acquire(&lvgl_api_lock);
    oled_demo_ui(display);
    _lock_release(&lvgl_api_lock);
}
